...
ALTER 	TABLE TEAM /* 수정 선언 */
DROP 	FOREIGN KEY FK_STADIUM_TEAM;  /* FK_STADIUM_TEAM 테이블 삭제 */
...
CREATE 	TABLE TEMP AS   /* TEMPORARY, AS 키워드는 항상 생략 가능 */
SELECT	*
FROM	SCHEDULE
WHERE	HOMETEAM_ID = 'K08' OR AWAYTEAM_ID = 'K08';  /* K08이 뛴 경기 검색 */

SELECT	* FROM TEMP;

DESCRIBE TEMP;  /* 테이블 정의 */
...
SELECT 	*
FROM 	INFORMATION_SCHEMA.SCHEMATA; /* 모든 culumn을 다 보겠다. */
...
SELECT 	*  /* column */
FROM 	INFORMATION_SCHEMA.TABLES  /* operand */
WHERE	TABLE_SCHEMA = 'kleague';   /* condition */
/* TABLES에서 TABLE_SCHEMA = 'kleague' 인 값들만 select */
/* 프로젝션*(셀렉트TABLE_SCHEMA = 'kleague'(TABLES)) */
...
ORDER   BY CONSTRAINT_NAME DESC;  /* CONSTRAINT_NAME을 기준으로 내림차순 정리 */

SELECT 	CONSTRAINT_SCHEMA, CONSTRAINT_NAME, DELETE_RULE, UPDATE_RULE, 
		TABLE_NAME, REFERENCED_TABLE_NAME  /* 6개의 컬럼만 select */
FROM 	INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS
WHERE	CONSTRAINT_SCHEMA = 'kleague' AND TABLE_NAME = 'schedule'; 
