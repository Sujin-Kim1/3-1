...
DROP TABLE IF EXISTS k_employee;
DROP TABLE IF EXISTS k_dependent;
DROP TABLE IF EXISTS k_department;
DROP TABLE IF EXISTS k_dept_locations;
DROP TABLE IF EXISTS k_project;
DROP TABLE IF EXISTS k_works_on;
...
CREATE TABLE k_employee (
    ...
    /* 스키마 변경을 하기 위해서 이름 작성하는 것이 용이함 */
    /* 이름이 있어야 DROP 가능 */
	CONSTRAINT 	fk_k_employee_k_employee	FOREIGN KEY (Super_ssn)	REFERENCES k_employee (Ssn)
    										ON DELETE CASCADE
											ON UPDATE CASCADE,
	CONSTRAINT 	fk_k_department_k_employee	FOREIGN KEY (Dno)		REFERENCES k_department (Dnumber) 
											ON DELETE CASCADE
											ON UPDATE CASCADE
											/* FOREIGN_KEY_CHECKS=1이면, 두번째 FK 제약조건에서 에러 발생함. */
);

CREATE TABLE k_dependent (
	...
	CONSTRAINT		fk_k_employee_k_dependent	FOREIGN KEY (Essn)	REFERENCES k_employee (Ssn)
												ON DELETE CASCADE
												ON UPDATE CASCADE
);
...
CREATE TABLE k_department (
	...
	CONSTRAINT 		fk_k_employee_k_department	FOREIGN KEY (Mgr_ssn)	REFERENCES k_employee (Ssn)
												ON DELETE CASCADE
												ON UPDATE CASCADE
);

CREATE TABLE k_dept_locations (
	Dnumber			INT(11) 	NOT NULL,
	Dlocation		VARCHAR(15) NOT NULL,
	PRIMARY KEY 	(Dnumber, Dlocation),
	CONSTRAINT 		fk_departtment_locations	FOREIGN KEY (Dnumber)	REFERENCES k_department (Dnumber)
												ON DELETE CASCADE
												ON UPDATE CASCADE
);
...
CREATE TABLE k_project (
	...
	CONSTRAINT 		fk_k_department_k_project	FOREIGN KEY (Dnum)	REFERENCES k_department (Dnumber)
												ON DELETE CASCADE
												ON UPDATE CASCADE
);

CREATE TABLE k_works_on (
	Essn 			CHAR(9) 	NOT NULL,
	Pno	 			INT(11) 	NOT NULL,
	Hours			DECIMAL(3,1),
	PRIMARY KEY 	(Essn, Pno),
	INDEX 			idx_Pno 	(Pno ASC),
	CONSTRAINT 		fk_k_employee_k_works_on 	FOREIGN KEY (Essn) 	REFERENCES k_employee (Ssn)
												ON DELETE CASCADE
												ON UPDATE CASCADE,  
    CONSTRAINT 		fk_k_project_k_works_on		FOREIGN KEY (Pno)	REFERENCES k_project (Pnumber)
												ON DELETE CASCADE
												ON UPDATE CASCADE
);
...
--  Data for k_employee (8 instances)

INSERT INTO k_employee VALUES
...
INSERT INTO k_dependent VALUES
...
--  Data for k_department (3 instances)

INSERT INTO k_department VALUES
(1, 'Headquarters', '888665555', '1981-06-19'),
(4, 'Administration', '987654321', '1995-01-01'),
(5, 'Research', '333445555', '1988-05-22');

--  Data for k_dept_locations (5 instances)

INSERT INTO k_dept_locations VALUES
(1, 'Houston'),
(4, 'Stafford'),
(5, 'Bellaire'),
(5, 'Houston'),
...
--  Data for k_project (6 instances)

INSERT INTO k_project VALUES
...
--  Data for k_works_on (16 instances)

INSERT INTO k_works_on VALUES
('123456789', 1, 32.5),
('123456789', 2, 7.5),
('333445555', 2, 10.0),
('333445555', 3, 10.0),
